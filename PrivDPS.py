...
class DirichletPosteriorSampling():
    ...
    def sample(self, x):
        return np.random.dirichlet(x+self.alpha)
...
class GaussianMechanism():
    ...
    def add_noises(self, x, d, n_trials): #x must be a (n_trials x d) array
        return x+np.random.normal(0, self.sigma, (n_trials,d))
